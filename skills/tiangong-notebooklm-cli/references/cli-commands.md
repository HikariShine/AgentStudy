...
Generate AI-powered video explainers from notebook content.

```bash
# Generate video with instructions
node {baseDir}/scripts/notebooklm.mjs generate video "Explain the key concepts" --notebook <notebook_id>

# Generate with specific format
node {baseDir}/scripts/notebooklm.mjs generate video "Create a brief overview" --format brief --notebook <notebook_id>

# Generate with style
node {baseDir}/scripts/notebooklm.mjs generate video "Explain the main ideas" --style anime --notebook <notebook_id>
```

**Video options:**
- `--format [explainer|brief]` - explainer (detailed) or brief (summary)
- `--style [auto|classic|whiteboard|kawaii|anime|watercolor|retro-print|heritage|paper-craft]`

**Download video:**
```bash
node {baseDir}/scripts/notebooklm.mjs download video ./output.mp4 --notebook <notebook_id> --latest
```

**Note:** Video generation takes 15-45 minutes. Use `--wait` or check status with `artifact list`.
